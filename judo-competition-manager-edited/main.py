...
from slave_window     import SlaveWindow, ModeDialog
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ── Restore in-progress match if found ────────────────────────────
        self._restore_inprogress_match()
    ...
    def _restore_inprogress_match(self):
        """Restore an unsaved in-progress match from the last session."""
        ...
        msg = (f"تم العثور على مباراة لم تكتمل من الجلسة السابقة:\n\n"
               f"الفئة: {cat or '—'}\n"
               f"الوقت المتبقي: {m:02d}:{s:02d}\n\n"
               "هل تريد استعادة المباراة؟")
        ans = QMessageBox.question(
            self, "استعادة المباراة", msg,
        ...
        if ans != QMessageBox.StandardButton.Yes:
            ...
            return
        self.engine.restore_from_state(state)
        # Sync UI in the match tab
        if hasattr(self, "match_tab"):
            self.match_tab.cat_edit.setText(state.get("category", ""))
    ...
    def _on_engine_update(self):
        try:
            ...
            # ── Continuous autosave for slave polling ────────────────────
            if (self.engine.white_id is not None or self.engine.blue_id is not None
                    or self.engine.running or self.engine.finished):
                db.save_inprogress_match(self.engine.to_state_dict())
    ...
    def closeEvent(self, event):
        ...
        db.save_settings(s)
        # ── Auto-save in-progress match ──────────────────────────────────
        if not self.engine.finished and (
            self.engine.white_id is not None or self.engine.blue_id is not None
        ):
            db.save_inprogress_match(self.engine.to_state_dict())
...
def main():
    ...
    # ── Mode selection ────────────────────────────────────────────────────
    mode_dlg = ModeDialog()
    if mode_dlg.exec() != QDialog.Accepted or mode_dlg.result_mode is None:
        return

    if mode_dlg.result_mode == ModeDialog.MODE_SLAVE:
        # ── SLAVE MODE ────────────────────────────────────────────────────
        win = SlaveWindow()
        win.show()
        sys.exit(app.exec())
