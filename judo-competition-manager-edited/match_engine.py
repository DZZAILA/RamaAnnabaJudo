...
class MatchEngine:
    ...
    def to_state_dict(self) -> dict:
        """Snapshot the full live match state (including timer) for persistence."""
        return {
            ...
            "golden":           self.golden,
            "golden_elapsed":   self.golden_elapsed,
            "finished":         self.finished,
            "winner":           self.winner,
            "running":          False,   # always pause on restore
            "osaekomi":         self.osaekomi,
            "osaekomi_elapsed": self.osaekomi_elapsed,
            "osaekomi_paused":  self.osaekomi_paused,
            "white": {
                "ippon":       self.white.ippon,
                "wazaari":     self.white.wazaari,
                "yuko":        self.white.yuko,
                "shido":       self.white.shido,
                "hansokumake": self.white.hansokumake,
            },
            "blue": {
                "ippon":       self.blue.ippon,
                "wazaari":     self.blue.wazaari,
                "yuko":        self.blue.yuko,
                "shido":       self.blue.shido,
                "hansokumake": self.blue.hansokumake,
            },
            "events": [{"type": e.event_type, "side": e.side, "time": e.match_time}
                       for e in self.events],
        }

    def restore_from_state(self, state: dict):
        """Restore engine state from a to_state_dict() snapshot."""
        self.white_id        = state.get("white_id")
        self.blue_id         = state.get("blue_id")
        self.category        = state.get("category", "")
        self.stage           = state.get("stage", "FINAL")
        self.time_left       = state.get("time_left", self.match_duration)
        self.golden          = state.get("golden", False)
        self.golden_elapsed  = state.get("golden_elapsed", 0)
        self.finished        = state.get("finished", False)
        self.winner          = state.get("winner")
        self.running         = False
        self.osaekomi        = state.get("osaekomi")
        self.osaekomi_elapsed = float(state.get("osaekomi_elapsed", 0))
        self.osaekomi_paused = state.get("osaekomi_paused", False)
        for side in ("white", "blue"):
            sc_data = state.get(side, {})
            sc = getattr(self, side)
        ...
        self.events = [
            MatchEvent(event_type=e["type"], side=e["side"], match_time=e.get("time", 0))
